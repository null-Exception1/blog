---
title: RSQR - A better architectural design approach to StreamingLLM
...
1. At a micro scale in each **block**, every delta-th token is kept as a survivor token and it's cache has a duplicate copy of precomputed cache of what would happen after the **swap**.

2. After exiting that **block**, the swap happens and all the precomputed **survivors** after the swap slot right into their respective positions as we had already precomputed for those **positions**.

3. Rinse and repeat when you exit the next block. 

4. The size of **block** and **delta** are specified ahead of time, its not a dynamic value.
...
1. After the block wise cache eviction strategy i actually decided that since blockwise eviction would be inconsistent for accuracy and be depended on block size it would be unclean architectural - purely an intuitive choice
...
2. Then i realised that RoPE actually dodges error if you only rotate an unrotated block it gives MASSIVELY less drift as compared to doing it twice or thrice. This must be why StreamingLLM is precise and still works.
...
3. Not only that but now i could confirm why exactly the gap between B_correct and B_uncorrected was a consistent gap not a compounding gap over n_cycles increasing.
...
RAW unrotated survivor tokens (global positions shown) behind the window
    |
T5 T10 T15 [ T16 T17 T18 T19... Tlatest ]
               | 
            ROTATED window shown with global positions 

AT RUNTIME they become

T13 T14 T15 [T16 T17 .. Tlatest]

Now using MiniPIC's rotating Query idea (since they're all relative to each other)

T1 T2 T3 [T4 T5 ... Tlatest]
